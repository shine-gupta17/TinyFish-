...
CLIENT_SECRETS_FILE = get_credential_file("google_forms")
SCOPES = get_platform_scopes("google_forms")
REDIRECT_URI = f"{BACKEND_URL}/auth/google-forms/oauth2callback"


@router.get("/login")
def google_forms_login(user_id: str, return_url: str = None) -> RedirectResponse:
    """Redirect user to Google OAuth2 consent screen for Forms access"""
    if not os.path.exists(CLIENT_SECRETS_FILE):
        raise HTTPException(
            status_code=500,
            detail=f"Google Forms OAuth credentials not found at {CLIENT_SECRETS_FILE}"
        )
    ...
    logger.info(f"Redirecting to Google Forms auth URL for user_id: {user_id}")
    return RedirectResponse(auth_url)


@router.get("/oauth2callback")
def oauth2callback(request: Request, code: str, state: str, scope: str = None) -> RedirectResponse:
    """Handle Google Forms OAuth2 callback and save creds to Supabase"""
    ...
    try:
        ...
        logger.info(f"Google Forms OAuth successful for user {user_id}")
        logger.info(f"Granted scopes saved: {granted_scopes}")
        
        separator = '&' if '?' in return_url else '?'
        final_redirect_url = f"{return_url}{separator}auth_success=google_forms"
        
        return RedirectResponse(url=final_redirect_url)
    
    except Exception as e:
        logger.error(f"Error during Google Forms OAuth callback: {e}", exc_info=True)
        return RedirectResponse(url=f"{return_url}?error=unexpected_error")


@router.get("/forms")
def read_forms(user_id: str):
    """Fetch Google Forms owned by the user"""
    try:
        # Get credentials from database
        result = supabase.table("connected_accounts").select("*").eq(
            "provider_id", user_id
        ).eq("platform", "google_forms").execute()

        if not result.data:
            return {"error": "❌ Google Forms not connected. Please authenticate first."}
        ...
        # Check if user granted necessary scopes for accessing forms
        required_scopes = [
            "https://www.googleapis.com/auth/forms.body",
            "https://www.googleapis.com/auth/forms.body.readonly",
            "https://www.googleapis.com/auth/drive.file"
        ]
        has_forms_permission = any(scope in granted_scopes for scope in required_scopes)
        
        if not has_forms_permission:
            return {
                "error": "❌ Insufficient permissions. Please re-authenticate and grant Forms access.",
        ...
        # Use Drive API to search for Google Forms
        drive_service = build("drive", "v3", credentials=creds)

        # Fetch forms (Google Forms have mimeType 'application/vnd.google-apps.form')
        ...
        for form in forms:
            forms_list.append({
                "id": form["id"],
                "name": form.get("name", ""),
                "webViewLink": form.get("webViewLink", ""),
        ...
        logger.error(f"Error reading forms: {e}")
        
        # Check if it's a permission error
        error_str = str(e).lower()
        if 'insufficient' in error_str or 'permission' in error_str or '403' in error_str:
            return {
                "error": "❌ Permission denied. Please re-authenticate and grant required Forms permissions.",
                "details": str(e)
            }
        
        return {"error": "Failed to fetch forms", "details": str(e)}


@router.get("/scopes")
def get_granted_scopes(user_id: str):
    """Check which Google Forms scopes the user has granted"""
    try:
        ...
        if not result.data:
            return {
                "connected": False,
                "error": "Google Forms not connected"
            }

        account = result.data[0]
        granted_scopes = account.get("scopes", [])
        
        # Define all possible Google Forms scopes
        all_scopes = {
        ...
        return {
            ...
            "can_read_forms": any(s in granted_scopes for s in [
                "https://www.googleapis.com/auth/forms.body",
                "https://www.googleapis.com/auth/forms.body.readonly",
                "https://www.googleapis.com/auth/drive.file"
            ]),
            "can_read_responses": "https://www.googleapis.com/auth/forms.responses.readonly" in granted_scopes
        }
...
def get_connection_status(user_id: str):
    """Check if Google Forms is connected for a user"""
    try:
        result = supabase.table("connected_accounts").select("*").eq(
            "provider_id", user_id
        ).eq("platform", "google_forms").execute()
        ...
        logger.error(f"Error checking Google Forms connection status: {e}")
        return {
            "connected": False,
            "error": str(e)
        }
...
def update_scopes(user_id: str = Query(...), scopes: List[str] = Query(...), return_url: str = Query(None)):
    ...
    try:
        ...
        logger.error(f"Error updating scopes for Google Forms: {e}")
...
def revoke_scope(user_id: str = Query(...), scope: str = Query(...), return_url: str = Query(None)):
    ...
    try:
        ...
        if not result.data:
            raise HTTPException(status_code=404, detail="Google Forms account not connected")
        ...
        logger.error(f"Error revoking scope for Google Forms: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def add_scope(user_id: str = Query(...), scope: str = Query(...), return_url: str = Query(None)):
    ...
    try:
        ...
        logger.error(f"Error adding scope for Google Forms: {e}")
        raise HTTPException(status_code=500, detail=str(e))
