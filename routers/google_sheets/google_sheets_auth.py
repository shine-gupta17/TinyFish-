...
CLIENT_SECRETS_FILE = get_credential_file("google_sheets")
SCOPES = get_platform_scopes("google_sheets")
REDIRECT_URI = f"{BACKEND_URL}/auth/google-sheets/oauth2callback"


@router.get("/login")
def google_sheets_login(user_id: str, return_url: str = None) -> RedirectResponse:
    """Redirect user to Google OAuth2 consent screen for Sheets access"""
    if not os.path.exists(CLIENT_SECRETS_FILE):
        raise HTTPException(
            status_code=500,
            detail=f"Google Sheets OAuth credentials not found at {CLIENT_SECRETS_FILE}"
        )
    ...
    logger.info(f"Redirecting to Google Sheets auth URL for user_id: {user_id}")
    return RedirectResponse(auth_url)


@router.get("/oauth2callback")
def oauth2callback(request: Request, code: str, state: str, scope: str = None) -> RedirectResponse:
    """Handle Google Sheets OAuth2 callback and save creds to Supabase"""
    ...
    try:
        ...
        logger.info(f"Google Sheets OAuth successful for user {user_id}")
        logger.info(f"Granted scopes saved: {granted_scopes}")
        
        separator = '&' if '?' in return_url else '?'
        final_redirect_url = f"{return_url}{separator}auth_success=google_sheets"
        
        return RedirectResponse(url=final_redirect_url)
    
    except Exception as e:
        logger.error(f"Error during Google Sheets OAuth callback: {e}", exc_info=True)
        return RedirectResponse(url=f"{return_url}?error=unexpected_error")
    

@router.get("/spreadsheets")
def read_spreadsheets(user_id: str):
    """Fetch last 5 Google Spreadsheets"""
    try:
        # Get credentials from database
        result = supabase.table("connected_accounts").select("*").eq(
            "provider_id", user_id
        ).eq("platform", "google_sheets").execute()
        ...
        # Check if user granted necessary scopes for accessing sheets
        required_scopes = [
            "https://www.googleapis.com/auth/spreadsheets",
            "https://www.googleapis.com/auth/drive.file"
        ]
        has_sheets_permission = any(scope in granted_scopes for scope in required_scopes)
        
        if not has_sheets_permission:
            return {
                "error": "❌ Insufficient permissions. Please re-authenticate and grant Sheets access.",
        ...
        for sheet in spreadsheets:
            sheet_data = drive_service.files().get(
                fileId=sheet["id"], fields="id,name,webViewLink").execute()
            name = sheet_data.get("name", "")
            sheets_list.append({"id": sheet["id"], "name": name})

        return sheets_list

    except Exception as e:
        logger.error(f"Error reading spreadsheets: {e}")
        
        # Check if it's a permission error
        error_str = str(e).lower()
        if 'insufficient' in error_str or 'permission' in error_str or '403' in error_str:
            return {
                "error": "❌ Permission denied. Please re-authenticate and grant required Sheets permissions.",
                "details": str(e)
            }
        
        return {"error": "Failed to fetch spreadsheets", "details": str(e)}


@router.get("/scopes")
def get_granted_scopes(user_id: str):
    """Check which Google Sheets scopes the user has granted"""
    try:
        ...
        # Define all possible Google Sheets scopes
        all_scopes = {
        ...
        return {
            ...
            "can_read_sheets": any(s in granted_scopes for s in [
                "https://www.googleapis.com/auth/spreadsheets",
                "https://www.googleapis.com/auth/drive.file"
            ])
        }
...
def get_connection_status(user_id: str):
    ...
    try:
        ...
        logger.error(f"Error checking Google Sheets connection status: {e}")
        return {
            "connected": False,
            "error": str(e)
        }
...
def update_scopes(user_id: str = Query(...), scopes: List[str] = Query(...), return_url: str = Query(None)):
    ...
    try:
        ...
        logger.error(f"Error updating scopes for Google Sheets: {e}")
...
def revoke_scope(user_id: str = Query(...), scope: str = Query(...), return_url: str = Query(None)):
    ...
    try:
        ...
        if not result.data:
            raise HTTPException(status_code=404, detail="Google Sheets account not connected")
        ...
        logger.error(f"Error revoking scope for Google Sheets: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def add_scope(user_id: str = Query(...), scope: str = Query(...), return_url: str = Query(None)):
    ...
    try:
        ...
        logger.error(f"Error adding scope for Google Sheets: {e}")
        raise HTTPException(status_code=500, detail=str(e))
