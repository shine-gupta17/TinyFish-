...
CLIENT_SECRETS_FILE = get_credential_file("google_meet")
SCOPES = get_platform_scopes("google_meet")
REDIRECT_URI = f"{BACKEND_URL}/auth/google-meet/oauth2callback"


@router.get("/login")
def google_meet_login(user_id: str, return_url: str = None) -> RedirectResponse:
    """Redirect user to Google OAuth2 consent screen for Meet access"""
    if not os.path.exists(CLIENT_SECRETS_FILE):
        raise HTTPException(
            status_code=500,
            detail=f"Google Meet OAuth credentials not found at {CLIENT_SECRETS_FILE}"
        )
    ...
    logger.info(f"Redirecting to Google Meet auth URL for user_id: {user_id}")
    return RedirectResponse(auth_url)


@router.get("/oauth2callback")
def oauth2callback(request: Request, code: str, state: str, scope: str = None) -> RedirectResponse:
    """Handle Google Meet OAuth2 callback and save creds to Supabase"""
    ...
    try:
        ...
        logger.info(f"Google Meet OAuth successful for user {user_id}")
        logger.info(f"Granted scopes saved: {granted_scopes}")
        
        separator = '&' if '?' in return_url else '?'
        final_redirect_url = f"{return_url}{separator}auth_success=google_meet"

        return RedirectResponse(url=final_redirect_url)

    except Exception as e:
        logger.error(f"Error during Google Meet OAuth callback: {e}", exc_info=True)
        return RedirectResponse(url=f"{return_url}?error=unexpected_error")


@router.get("/meetings")
def read_meetings(user_id: str, max_results: int = 10):
    """Fetch upcoming Google Meet meetings from Calendar"""
    try:
        # Get credentials from database
        result = supabase.table("connected_accounts").select("*").eq(
            "provider_id", user_id
        ).eq("platform", "google_meet").execute()

        if not result.data:
            return {"error": "❌ Google Meet not connected. Please authenticate first."}
        ...
        meet_events = []
        for event in events:
            if 'conferenceData' in event and event['conferenceData'].get('conferenceSolution', {}).get('key', {}).get('type') == 'hangoutsMeet':
                ...
                meet_events.append({
                    ...
                    "conferenceId": event['conferenceData'].get('conferenceId', '')
                })

        return meet_events

    except Exception as e:
        logger.error(f"Error reading meetings: {e}")
        ...
        return {"error": "Failed to fetch meetings", "details": str(e)}


@router.get("/scopes")
def get_granted_scopes(user_id: str):
    """Check which Google Meet scopes the user has granted"""
    try:
        ...
        # Define all possible Google Meet scopes
        all_scopes = {
...
def get_connection_status(user_id: str):
    """Check if Google Meet is connected for a user"""
    try:
        result = supabase.table("connected_accounts").select("*").eq(
            "provider_id", user_id
        ).eq("platform", "google_meet").execute()
        ...
        logger.error(f"Error checking Google Meet connection status: {e}")
        return {
            "connected": False,
            "error": str(e)
        }
...
def update_scopes(user_id: str = Query(...), scopes: list[str] = Query(...), return_url: str = Query(None)):
    ...
    try:
        ...
        logger.error(f"Error updating scopes for Google Meet: {e}")
...
def revoke_scope(user_id: str = Query(...), scope: str = Query(...), return_url: str = Query(None)):
    ...
    try:
        ...
        if not result.data:
            raise HTTPException(status_code=404, detail="Google Meet account not connected")
        ...
        logger.error(f"Error revoking scope for Google Meet: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
def add_scope(user_id: str = Query(...), scope: str = Query(...), return_url: str = Query(None)):
    ...
    try:
        ...
        logger.error(f"Error adding scope for Google Meet: {e}")
        raise HTTPException(status_code=500, detail=str(e))
